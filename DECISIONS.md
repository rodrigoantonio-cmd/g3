...
| 2026-07-29 | **Salvar/histórico no Supabase** | Botão "Salvar campanha" grava em campaigns+briefings+assets (RLS por usuário); `/campanhas` lista. Sem mudança de schema. |
| 2026-07-29 | **Fase 4: base de consulta (referencias)** | Tabela `referencias` aplicada no banco; `lib/referencias.ts` + `/api/ingest`; `/api/generate` e `/api/copies` anexam a base ao contexto. Semeada com 8 itens do histórico (284 + ISS SP + swipe file). Ingestão completa da pasta do Drive fica como tarefa recorrente. |
| 2026-07-29 | **Repositório git iniciado + commit inicial** | 47 arquivos versionados; `.env.local` fora (gitignore). Deploy no Vercel documentado em `DEPLOY.md` — pendente das contas GitHub/Vercel do dono. |
