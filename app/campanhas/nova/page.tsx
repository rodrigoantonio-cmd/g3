...
export default function NovaCampanhaPage() {
  ...
  function atualizarCelula(bloco: BlocoArray, i: number, key: string, valor: string) {
    setEstrutura((e) => {
      if (!e) return e;
      const linhas = (e[bloco] as unknown as Linha[]).map((l) => ({ ...l }));
      linhas[i] = { ...linhas[i], [key]: valor };
      return { ...e, [bloco]: linhas } as CampanhaEstruturada;
    });
  }
  ...
  function adicionarLinha(bloco: BlocoArray) {
    setEstrutura((e) => {
      ...
      const linhas = [...(e[bloco] as unknown as Linha[]), vazio];
      return { ...e, [bloco]: linhas } as CampanhaEstruturada;
    });
  }
  ...
  function removerLinha(bloco: BlocoArray, i: number) {
    setEstrutura((e) => {
      if (!e) return e;
      const linhas = (e[bloco] as unknown as Linha[]).filter((_, idx) => idx !== i);
      return { ...e, [bloco]: linhas } as CampanhaEstruturada;
    });
  }
  ...
  function atualizarCapa(key: string, valor: string) {
    setEstrutura((e) =>
      e ? ({ ...e, capa: { ...e.capa, [key]: valor } } as CampanhaEstruturada) : e
    );
  }
      ...
      {etapa === 3 && estrutura && (
        <div>
          <div className="card">
            ...
            <p className="subtitulo">Edite os campos livremente. As mudanças entram na planilha e nas copies.</p>
            <div className="linha" style={{ gap: 16 }}>
              {Object.keys(estrutura.capa).map((key) => (
                <div key={key} style={{ flex: "1 1 240px" }}>
                  <label htmlFor={`capa-${key}`}>{CAPA_LABELS[key] ?? key}</label>
                  <input
                    id={`capa-${key}`}
                    value={(estrutura.capa as unknown as Linha)[key] ?? ""}
          ...
          {(Object.keys(COLUNAS) as BlocoArray[]).map((bloco) => {
            const linhas = estrutura[bloco] as unknown as Linha[];
